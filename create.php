...
// select box -> the array of gerer code 
try {
  $pdo = new PDO($dsn, $user, $password);

  //genres table -> get date of genre code
    ...
    <article class="registration">
      ...
      <form action="create.php" method="post" class="registration-form">
        <div>
          ...
          <input type="number" id="book_code" name="book_code" min="0" max="100000" required>

          <label for="book_name">書籍名</label>
          <input type="text" id="book_name" name="book_name" maxlength="50" required>

          <label for="price">値段</label>
          <input type="number" id="price" name="price" min="0" max="100000" required>

          <label for="stock_quantity">在庫数</label>
          <input type="number" id="stock_quantity" name="stock_quantity" min="0" max="100000" required>

          <label for="genre_code">ジャンルコード</label>
          <select id="genre_code" id="genre_code" name="genre_code" required>
            <option disabled selected value>選択してください</option>
            <?php
            // get array into select box
            foreach ($genre_codes as $genre_code) {
              echo "<option value='{$genre_code}'>{$genre_code}";
            }
            ?>
          </select>
        </div>
